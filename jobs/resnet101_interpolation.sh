#!/bin/bash
#SBATCH --job-name=resnet101_interp
...
#SBATCH --output=logs/resnet101_interpolation_%j.out
#SBATCH --error=logs/resnet101_interpolation_%j.err

# Stage-3 residual schedule sweep on pretrained ResNet-101.
# Experiment list lives in scripts/resnet101_interpolation.py (EXPERIMENTS).
#
# Submit from the repo root:
#   source .env && sbatch --account="$SLURM_ACCOUNT" jobs/resnet101_interpolation.sh
# Or simply:
#   bash jobs/resnet101_interpolation.sh
...
SCRIPT="${PROJECT_ROOT}/scripts/resnet101_interpolation.py"
