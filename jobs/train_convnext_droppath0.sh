#!/bin/bash
#SBATCH --job-name=train_convnext_dp0
...
#SBATCH --output=logs/train_convnext_droppath0_%j.out
#SBATCH --error=logs/train_convnext_droppath0_%j.err

# Same recipe as train_convnextV1, with stochastic depth disabled (drop_path_rate=0).
#
# Submit from the repo root:
#   source .env && sbatch --account="$SLURM_ACCOUNT" jobs/train_convnext_droppath0.sh
#
...
# Job-specific knobs: set after sourcing .env so a leftover EXPERIMENT_NAME there
# does not resume the drop_path=0.1 run.
DROP_PATH_RATE=0
EXPERIMENT_NAME="convnextv1_imagenet_droppath0"
EXPERIMENT="${PROJECT_ROOT}/outputs/${EXPERIMENT_NAME}"
RETAKE="${RETAKE:-0}"
export DROP_PATH_RATE EXPERIMENT_NAME RETAKE
...
TRAIN_SCRIPT="${TRAIN_SCRIPT:-scripts/train_convnextV1.py}"
