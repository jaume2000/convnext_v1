...
STAGE3_LENGTH = 27  # one shared block looped 27 times (vs 9 in the D9 shared baseline)
USE_DDP = True
EPOCHS = 300
WARMUP_EPOCHS = 10
BATCH_SIZE = 256
...
model = DeltaConvNext(stage3_length=STAGE3_LENGTH, useDeltas=USE_DELTAS)
model.rewire()
model = model.to(device)
if rank == 0 and not USE_DELTAS:
    n_delta = sum(1 for n, _ in model.named_parameters() if "Delta" in n)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"Shared-only D{STAGE3_LENGTH}: {n_params:,} params, {n_delta} delta params on GPU")
if use_ddp:
