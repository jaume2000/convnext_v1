...
USE_DELTAS = False  # shared-only: 1 block per stage, looped [3, 3, 9, 3]
USE_DDP = True
EPOCHS = 300
WARMUP_EPOCHS = 10
BATCH_SIZE = 256
...
model.rewire_all_stages()  # 4 shared blocks, looped [3, 3, 9, 3]
model = model.to(device)
if rank == 0 and not USE_DELTAS:
    n_delta = sum(1 for n, _ in model.named_parameters() if "Delta" in n)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"All-stages shared: {n_params:,} params, {n_delta} delta params on GPU")
    print(f"Shared blocks: {list(model.sharedBlocks.keys())} (channels {[96, 192, 384, 768]})")
if use_ddp:
