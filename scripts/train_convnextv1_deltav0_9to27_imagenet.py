...
# Shared checkpoint is D=9; we unroll to 27 steps with EU = 9/27 = 1/3 and train deltas.
STAGE3_LENGTH = 27
USE_DELTAS = True
USE_DDP = True
EPOCHS = 100
...
# Load the shared D=9 checkpoint first, then unroll to 27 steps (EU=1/3) with deltas.
# Expanding before the load would misalign tail keys (indices 9/10 vs 27/28).
...
if not RETAKE:
    ...
    stDict = torch.load(CHECKPOINT, map_location="cpu")
    # Shared checkpoint has no tail.* (aliases of deltifiedStage3.9/10) and no *Delta
    # params (zero-init here, so the model matches the shared baseline at epoch 0).
    incompatible = model.load_state_dict(stDict["model"], strict=False)
    tail_aliases = {
        f"tail.{index}.{part}"
        for index in (model.depths[2], model.depths[2] + 1)
        for part in ("weight", "bias")
    }
    allowed_missing = tail_aliases | {
        k for k in incompatible.missing_keys if "Delta" in k
    }
    stale = set(incompatible.missing_keys) - allowed_missing
    if stale or incompatible.unexpected_keys:
        raise RuntimeError(
            f"Refusing to train on a partial load of {CHECKPOINT}: "
            f"missing {sorted(stale)}, "
            f"unexpected {sorted(incompatible.unexpected_keys)}"
        )
    if rank == 0:
        n_delta = sum(1 for k in incompatible.missing_keys if "Delta" in k)
        print(
            f"Loaded {CHECKPOINT} (epoch {stDict.get('epoch')}); "
            f"{n_delta} zero-init deltas on D=9; expanding to D={STAGE3_LENGTH}"
        )
model.setStage3Length(STAGE3_LENGTH)
if rank == 0 and not RETAKE:
    print(
        f"D={STAGE3_LENGTH}, eulerStep={model.eulerStep:g}; "
        f"epoch 1 val top1 must land near 0.80"
    )
...
param_groups = build_param_groups_with_delta_weight_decay(
    model, weight_decay=WEIGHT_DECAY, delta_weight_decay=DELTA_WEIGHT_DECAY
)
if rank == 0:
    ...
    if delta_groups:
        msg += (
            f", delta weight decay {DELTA_WEIGHT_DECAY} on "
            f"{sum(p.numel() for g in delta_groups for p in g['params'])} params"
        )
