...
if not RETAKE:
    ...
    # The checkpoint predates rewire() registering self.tail, so it carries no tail.* keys.
    # Those name the same two modules as deltifiedStage3.9/10, which the load does fill, so
    # strict=False is safe -- but only for exactly those four names. Any other gap means the
    # backbone is partly random, and with the whole backbone frozen nothing would ever fix
    # it: the run would burn days to converge onto garbage features.
    ...
    if set(incompatible.missing_keys) != tail_aliases or incompatible.unexpected_keys:
        raise RuntimeError(
            f"Refusing to train on a partial load of {CHECKPOINT}: "
            f"missing {sorted(set(incompatible.missing_keys) - tail_aliases)}, "
