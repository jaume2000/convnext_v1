"""Swin-T with a configurable stage-3 block schedule (Euler-scaled)."""
...
class InterpoledSwinT(nn.Module):
    ...
    always run once after that sequence. Each scheduled block is Euler-scaled:

        x <- x + euler_step * (stage3[i](x) - x)
        ...
        for i, module in enumerate(b.features):
            if i == self.STAGE3_INDEX:
                for j in self.blocks:
                    x = x + self.euler_step * (stage3[j](x) - x)
            else:
                x = module(x)
        x = b.norm(x)
        x = b.permute(x)
